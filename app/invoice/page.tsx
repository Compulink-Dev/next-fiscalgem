...
const Invoice = () => {
  ...
  if (!receiptData) {
    return (
      <div className="bg-green-900 h-screen w-screen flex flex-col items-center justify-center text-white">
  ...
  const totalTaxes = safeReceiptLines.reduce((sum, line) => {
    const lineTax =
      (line.receiptLinePrice * line.receiptLineQuantity * line.taxPercent) /
      (100 + line.taxPercent);
    return sum + lineTax;
  }, 0);
  ...
  return (
    <div className="m-8 max-w-5xl mx-auto p-6 text-sm font-sans">
      {/* Header Section */}
      ...
      <div className="text-center my-4">
        <h1 className="text-xl font-bold">
          {payload.receipt.receiptType === "FISCALINVOICE"
            ? "FISCAL TAX INVOICE"
            : payload.receipt.receiptType === "CREDITNOTE"
            ? "CREDIT NOTE"
            : payload.receipt.receiptType === "DEBITNOTE"
            ? "DEBIT NOTE"
      ...
      <div className="grid grid-cols-2 gap-6 mt-4">
        ...
        <div>
          ...
          <p>
            Phone: {payload.receipt.buyerData?.buyerContacts?.phoneNo || ""}
          </p>
          <p>
            Address:{" "}
            {`${payload.receipt.buyerData?.buyerAddress?.street || ""}, ${
              payload.receipt.buyerData?.buyerAddress?.city || ""
            }`}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6 mt-4">
        <div>
          <p>
            <strong>Invoice No:</strong> {payload.receipt.invoiceNo}
          </p>
          <p>
            <strong>Customer reference No:</strong> {payload.receipt.invoiceNo}
          </p>
          <p>
            <strong>Device Serial No:</strong> {payload.deviceID}
          </p>
        </div>
        <div>
          <p>
            <strong>Fiscal Day No:</strong> {payload.receipt.receiptGlobalNo}
          </p>
          <p>
            <strong>Date:</strong>{" "}
            {new Date(payload.receipt.receiptDate).toLocaleString()}
          </p>
          <p>
            <strong>Fiscal Device ID:</strong> {payload.deviceID}
          </p>
        </div>
      </div>
      <Separator className="my-4 h-[1.5px] rounded bg-black" />
              ...
              return (
                <tr key={index} className="border">
                  ...
                  <td className="border p-2 text-right">
                    {line.receiptLinePrice.toFixed(2)}
                  </td>
                  <td className="border p-2 text-right">
                    {lineTax.toFixed(2)}
                  </td>
                  <td className="border p-2 text-right">
                    {line.receiptLineTotal.toFixed(2)}
                  </td>
                </tr>
              );
            })
      ...
      <div className="flex items-end justify-between gap-4 w-full">
        ...
        <div className="flex-1 flex flex-row justify-between text-right gap-6">
          ...
          <p>
            {amountExclTax.toFixed(2)} {payload.receipt.receiptCurrency}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-end justify-between gap-4 w-full">
        ...
        <div className="flex-1 flex flex-row justify-between text-right gap-6">
          ...
          {/* <p>{safeReceiptTaxes[0]?.taxAmount.toFixed(2) || "0.00"} {receipt.receiptCurrency}</p> */}
          <p>
            {totalTaxes.toFixed(2)} {payload.receipt.receiptCurrency}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-end justify-between gap-4 w-full">
        ...
        {/* Separator for Signature */}. <div className="flex-1"></div>
        {/* Right Section for Amount */}
        <div className="flex-1 mt-4 font-bold flex flex-row items-center justify-between text-right gap-6">
          ...
          <p>
            {receiptTotal.toFixed(2)} {payload.receipt.receiptCurrency}
          </p>
        </div>
      </div>
      ...
      <div className="my-4 flex gap-4">
        <Button
          className="no-print bg-green-700 hover:bg-green-500"
