...
const receiptSchema = z.object({
    ...
    receiptGlobalNo: z.preprocess(val => Number(val) || 0, z.number().positive().min(1, "Global Number must be valid")),
    receiptDate: z.string().refine(
        date => !isNaN(Date.parse(date)),
        { message: "Invalid date format. Please use 'YYYY-MM-DDTHH:mm'" }
    ),
    receiptTotal: z.preprocess(val => Math.round(Number(val) || 0), z.number().min(1, "Total must be greater than zero")),
    receiptTaxes: z.array(
        z.object({
            taxID: z.preprocess(val => Number(val) || 0, z.number().positive("Tax ID must be a number")),
            taxCode: z.string().optional(),
            taxPercent: z.preprocess(
                val => parseFloat(val as string) || 0.0,
                z.number().optional()
            ),
            taxAmount: z.preprocess(val => Math.round(Number(val) || 0), z.number().min(0)),
            salesAmountWithTax: z.preprocess(val => Math.round(Number(val) || 0), z.number().min(0)),
        })
    ).optional(),
...
export default function ReceiptForm() {
    const { register, handleSubmit, formState: { errors }, control } = useForm<ReceiptData>({
    ...
    const [receiptSignature, setReceiptSignature] = useState<{ hash: string; signature: string } | null>(null);
    ...
    const onSubmit = async (data: ReceiptData) => {
        ...
        try {
            ...
            const paddedDeviceID = String(formattedData.deviceID).padStart(10
                , "0");
            ...
            const paddedGlobalNo = String(formattedData.receiptGlobalNo).padStart(10, "0");
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="border border-gray-300 p-6 rounded space-y-6 w-[500px]">
            <div className="space-y-2">
                <Label>Device ID</Label>
                <Input {...register("deviceID")} />
                {errors.deviceID && <p className="text-red-600 text-sm">{errors.deviceID.message}</p>}
            </div>
            <div>
                <Label>Receipt Type</Label>
                <Input {...register("receiptType")} />
                {errors.receiptType && <p className="text-xs text-red-700 my-2">{errors.receiptType.message}</p>}
            </div>
            <div>
                <Label>Receipt Currency</Label>
                <Input {...register("receiptCurrency")} />
                {errors.receiptCurrency && <p className="text-xs text-red-700 my-2">{errors.receiptCurrency.message}</p>}
            </div>
            <div>
                <Label>Receipt Global Number</Label>
                <Input type="number" {...register("receiptGlobalNo")} />
                {errors.receiptGlobalNo && <p className="text-xs text-red-700 my-2">{errors.receiptGlobalNo.message}</p>}
            </div>
            <div>
                <Label>Receipt Date</Label>
                <Input type="datetime-local" {...register("receiptDate")} />
                {errors.receiptDate && <p className="text-xs text-red-700 my-2">{errors.receiptDate.message}</p>}
            </div>
            <div>
                <Label>Receipt Total</Label>
                <Input type="number" step="0.01" {...register("receiptTotal")} />
                {errors.receiptTotal && <p className="text-xs text-red-700 my-2">{errors.receiptTotal.message}</p>}
            </div>
                ...
                {fields.map((item, index) => (
                    <div key={item.id}>
                        <div>
                            ...
                            {errors.receiptTaxes?.[index]?.taxID && <p className="text-xs text-red-700 my-2">{errors.receiptTaxes[index].taxID.message}</p>}
                        </div>
                        <div>
                            <Label>Tax Code</Label>
                            <Input {...register(`receiptTaxes.${index}.taxCode`)} />
                            {errors.receiptTaxes?.[index]?.taxCode && <p className="text-xs text-red-700 my-2">{errors.receiptTaxes[index].taxCode.message}</p>}
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <Input type="number" step="0.01" {...register(`receiptTaxes.${index}.taxAmount`)} />
                            {errors.receiptTaxes?.[index]?.taxAmount && <p className="text-xs text-red-700 my-2">{errors.receiptTaxes[index].taxAmount.message}</p>}
                        </div>
                        <div>
                            <Label>Sales Amount With Tax</Label>
                            <Input type="number" step="0.01" {...register(`receiptTaxes.${index}.salesAmountWithTax`)} />
                            {errors.receiptTaxes?.[index]?.salesAmountWithTax && <p>{errors.receiptTaxes[index].salesAmountWithTax.message}</p>}
                        </div>
                        <Button className="mt-4 bg-red-600 hover:bg-red-500 text-xs" type="button" onClick={() => remove(index)}>
                            <X />
                            <p className="">Remove Tax</p>
                        </Button>
                    </div>
                ))}
                <Button variant={'outline'} type="button" onClick={() => append({ taxID: 2, taxAmount: 0, taxPercent: 0, salesAmountWithTax: 0 })}>
                    <Plus />
            ...
            <div>
                ...
                {errors.previousReceiptHash && <p className="text-xs text-red-700 my-2">{errors.previousReceiptHash.message}</p>}
            </div>
                ...
                <div className="bg-green-400 text-green-950 rounded p-4 text-sm w-full">
                    ...
                    <div className="space-y-2">
                        <div className="flex gap-2">
                            <strong>Hash:</strong>
                            <span className="text-ellipsis text-sx overflow-hidden" style={{ wordBreak: "break-all" }}>
                        ...
                        {qrUrl && (
                            <div className="mt-2">
                                <p className="font-bold"> MD5 Hash: <span className="font-normal">{md5Hash16}</span></p>
                                <div className="my-4">
