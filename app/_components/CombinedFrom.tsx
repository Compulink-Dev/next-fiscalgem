...
const receiptSchema = z.object({
  ...
  receipt: z.object({
    ...
    receiptCurrency: z.string().min(1, "Currency is required"),
    receiptCounter: z.number().min(1, "Counter must be valid"),
    receiptGlobalNo: z.number().min(1, "Global Number must be valid"),
    invoiceNo: z.string().min(1, "Invoice Number is required"),
    buyerData: z.object({
      buyerRegisterName: z
        .string()
        .min(1, "buyerData.buyerRegisterName` is required."),
      buyerTradeName: z.string(),
      vatNumber: z
        .string()
        .min(9, "Vat Number must be min length of 9")
        .max(9, "Vat Number must be max length of 9")
        .optional(),
      buyerTIN: z
        .string()
        .min(10, "Buyer TIN must be min length of 10")
        .max(10, "Buyer TIN must be max length of 10")
        .optional(),
    ...
    receiptLines: z.array(
      z.object({
        receiptLineType: z.string().min(1, "Line Type is required"),
        receiptLineNo: z.number().min(1, "Line Number is required"),
        receiptLineHSCode: z
          .string()
          .max(8, "HS Code must be a string with a maximum length of 8")
          .optional(),
        receiptLineName: z.string().min(1, "Line Name is required"),
        receiptLinePrice: z.number().min(0, "Price must be valid"),
        receiptLineQuantity: z.number().min(1, "Quantity must be valid"),
        receiptLineTotal: z.number().min(0, "Total must be valid"),
        taxPercent: z.number().min(0, "Tax Percent must be valid"),
        taxCode: z
          .string()
          .min(0, "Tax Code must be valid")
          .max(3, "Tax Code must be a string with a maximum length of 3"),
...
export default function CombinedForm() {
  ...
  const {
    fields: receiptLines,
    append: appendReceiptLine,
    remove: removeReceiptLine,
  } = useFieldArray({
    control,
  ...
  const {
    fields: receiptTaxesFields,
    append: appendReceiptTax,
    remove: removeReceiptTax,
  } = useFieldArray({
    control,
  ...
  const {
    fields: receiptPayments,
    append: appendReceiptPayment,
    remove: removeReceiptPayment,
  } = useFieldArray({
    control,
  ...
  useEffect(() => {
    ...
    const updatedLines = receiptLines.map((line, index) => {
      ...
      setValue(
        `receipt.receiptLines.${index}.receiptLineTotal`,
        calculatedTotal,
        { shouldValidate: true }
      );
      ...
      const matchingTax = receiptTaxes.find(
        (tax) => tax.taxCode === line.taxCode
      );
      if (matchingTax) {
        const taxAmount = (calculatedTotal * line.taxPercent) / 100;

        setValue(
          `receipt.receiptTaxes.${index}.salesAmountWithTax`,
          calculatedTotal,
          { shouldValidate: true }
        );
        setValue(`receipt.receiptTaxes.${index}.taxAmount`, taxAmount, {
          shouldValidate: true,
        });
      } else {
        appendReceiptTax({
    ...
    const receiptTotal = updatedLines.reduce(
      (sum, line) => sum + line.receiptLineTotal,
      0
    );
  ...
  useEffect(() => {
    const updatedPayments = receiptPayments.map((payment, index) => {
      ...
      const correspondingTax = receiptTaxes.find(
        (tax) => tax.taxCode === payment.moneyTypeCode
      ); // Adjust key mapping if needed
      const newPaymentAmount = correspondingTax
        ? correspondingTax.salesAmountWithTax
        : 0;
  ...
  const handleAddReceiptLine = () => {
    const currentLineNo =
      receiptLines.length > 0
        ? receiptLines[receiptLines.length - 1].receiptLineNo
        : 0;
    const currentTaxCode =
      receiptLines.length > 0
        ? parseInt(receiptLines[receiptLines.length - 1].taxCode, 10)
        : 0;
  ...
  const onSubmit = async (data: ReceiptFormData) => {
    ...
    console.log(
      "Receipt Device Signature: ",
      data.receipt.receiptDeviceSignature
    );
    ...
    const calculateReceiptTotal = (
      receiptLines: ReceiptFormData["receipt"]["receiptLines"]
    ) => {
      let total = 0;
    ...
    try {
      ...
      data.receipt.receiptTaxes.forEach((tax) => {
        ...
        const relatedLines = data.receipt.receiptLines.filter(
          (line) =>
            line.taxCode === tax.taxCode && line.taxPercent === tax.taxPercent
        );
        console.log("Tax for receipt line: ", tax);

        // Sum up receiptLineTotal for the related lines
        const totalLineAmount = relatedLines.reduce(
          (sum, line) => sum + line.receiptLineTotal,
          0
        );
        ...
        if (data.receipt.receiptLinesTaxInclusive) {
          ...
          tax.taxAmount = parseFloat(
            (
              totalLineAmount *
              (tax.taxPercent / (100 + tax.taxPercent))
            ).toFixed(2)
          );
          ...
          tax.taxAmount = parseFloat(
            (totalLineAmount * (tax.taxPercent / 100)).toFixed(2)
          );
          tax.salesAmountWithTax = parseFloat(
            (totalLineAmount * (1 + tax.taxPercent / 100)).toFixed(2)
          );
        }
      });
      ...
      const paddedGlobalNo = String(data.receipt.receiptGlobalNo).padStart(
        10,
        "0"
      );
      ...
      console.log(
        "Receipt Lines before submission: ",
      ...
      if (submissionResult.success) {
        ...
        router.push(
          `/invoice?hash=${encodeURIComponent(
            result.receiptDeviceSignature.hash
          )}&signature=${encodeURIComponent(
            result.receiptDeviceSignature.signature
          )}&qrUrl=${encodeURIComponent(
            qrCodeUrl
          )}&md5Hash=${encodeURIComponent(
            md5Hash
          )}&payload=${encodeURIComponent(JSON.stringify(finalData))}`
        );
  ...
  const updateReceiptTotal = () => {
    const updatedTotal = receiptLines.reduce(
      (sum, line) => sum + (line.receiptLineTotal || 0),
      0
    );
  ...
  return (
    <div className="p-8 flex flex-col w-full items-center justify-center gap-8">
      <form
        ...
        className="w-[600px] border border-green-700 p-4 space-y-2 rounded"
      >
        <div className="">
          <Title />
        ...
        <div className="flex items-center justify-between gap-2">
          <FormInput
            ...
            type="text"
            register={register}
            error={errors.receipt?.invoiceNo}
          />
          <FormInput
            label="Receipt Date:"
            ...
            type="datetime-local"
            register={register}
            error={errors.receipt?.receiptDate}
          />
        </div>
        <div>
          <FormInput
            label="Receipt Notes:"
            name="receipt.receiptNotes"
            type="textarea"
            register={register}
            error={errors.receipt?.receiptNotes}
          />
        </div>
        ...
        <div className="flex items-center justify-between gap-2">
          <FormInput
            ...
            type="text"
            register={register}
            error={errors.receipt?.buyerData?.buyerRegisterName}
          />
          <FormInput
            label="Buyer Trade Name*:"
            name="receipt.buyerData.buyerTradeName"
            type="text"
            register={register}
            error={errors.receipt?.buyerData?.buyerTradeName}
          />
        </div>
        <div className="flex items-center justify-between gap-2">
          <FormInput
            label="VAT Number*:"
            name="receipt.buyerData.vatNumber"
            type="text"
            register={register}
            error={errors.receipt?.buyerData?.vatNumber}
          />
          <FormInput
            label="Buyer TIN*:"
            name="receipt.buyerData.buyerTIN"
            type="text"
            register={register}
            error={errors.receipt?.buyerData?.buyerTIN}
          />
        </div>
        <div className="flex items-center gap-2">
          <FormInput
            label="Phone Number*:"
            name="receipt.buyerData.buyerContacts.phoneNo"
            type="text"
            register={register}
            error={errors.receipt?.buyerData?.buyerContacts?.phoneNo}
          />
          <FormInput
            label="Email*:"
            name="receipt.buyerData.buyerContacts.email"
            type="email"
            register={register}
            error={errors.receipt?.buyerData?.buyerContacts?.email}
          />
        </div>
        <div className="flex items-center gap-2">
          <FormInput
            label="Province*:"
            name="receipt.buyerData.buyerAddress.province"
            type="text"
            register={register}
            error={errors.receipt?.buyerData?.buyerAddress?.province}
          />
          <FormInput
            label="City*:"
            name="receipt.buyerData.buyerAddress.city"
            type="text"
            register={register}
            error={errors.receipt?.buyerData?.buyerAddress?.city}
          />
        </div>
        <div className="flex items-center gap-2">
          <FormInput
            label="Street*:"
            name="receipt.buyerData.buyerAddress.street"
            type="text"
            register={register}
            error={errors.receipt?.buyerData?.buyerAddress?.street}
          />
          <FormInput
            label="House No*:"
            name="receipt.buyerData.buyerAddress.houseNo"
            type="text"
            register={register}
            error={errors.receipt?.buyerData?.buyerAddress?.houseNo}
          />
        </div>
        <FormInput
          label="District*:"
          name="receipt.buyerData.buyerAddress.district"
          type="text"
          register={register}
          error={errors.receipt?.buyerData?.buyerAddress?.district}
        />
        ...
        {receiptLines.map((item, index) => (
          <div key={item.id}>
            <div className="flex items-center gap-2">
              <FormSelect
                label="Receipt Line Type:"
                name={`receipt.receiptLines.${index}.receiptLineType`}
                options={[
              ...
              <FormInput
                ...
                name={`receipt.receiptLines.${index}.receiptLineNo`}
                type="number"
                register={(name: any) =>
                  register(name, {
                    setValueAs: (value) =>
                ...
                error={errors.receipt?.receiptLines?.[index]?.receiptLineNo}
              />
            </div>
            <div className="flex items-center gap-2">
              <FormInput
                label="Receipt Line HS Code*:"
                name={`receipt.receiptLines.${index}.receiptLineHSCode`}
                type="text"
                register={register}
                error={errors.receipt?.receiptLines?.[index]?.receiptLineHSCode}
              />
              <FormInput
                label="Receipt Line Name:"
                name={`receipt.receiptLines.${index}.receiptLineName`}
                type="text"
                register={register}
                error={errors.receipt?.receiptLines?.[index]?.receiptLineName}
              />
            </div>
            <div className="flex items-center gap-2">
              <FormTrigger<ReceiptFormData>
                ...
                trigger={trigger}
                error={
                  errors.receipt?.receiptLines?.[index]?.receiptLineQuantity
                }
                onChange={(value) => {
                  ...
                  const price =
                    parseInt(
                      getValues(
                        `receipt.receiptLines.${index}.receiptLinePrice`
              ...
              <FormTrigger<ReceiptFormData>
                ...
                onChange={(value) => {
                  ...
                  const quantity =
                    parseInt(
                      getValues(
                        `receipt.receiptLines.${index}.receiptLineQuantity`
                      ) as unknown as string,
                      10
                    ) || 0;
                  const total = price * quantity;
            ...
            <div className="flex items-center gap-2">
              <FormInput
                label="Tax Code*:"
                name={`receipt.receiptLines.${index}.taxCode`}
                type="text"
                register={register}
                error={errors.receipt?.receiptLines?.[index]?.taxCode}
              />
        ...
        <div className="w-full">
          <Button
            type="submit"
            className="bg-green-700 hover:bg-green-500 mt-4 w-full"
          >
